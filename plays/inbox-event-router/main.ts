 ...
 *       server_info: { name: Gmail API, version: discovery/v1 }
 ...
 *   required: false
 *   default: ""
 *   description: Schema-v1 agent decision envelope required in apply mode.
 * steps:
 *   prepare:
 *     type: process.exec
 *     argv: [deno, run, --allow-env, --allow-read, "@resource{prepare.ts}", $mode, $calendar_id, $lookback_days]
 *   auth_gmail:
 ...
 *       condition:
 *         all:
 *         - compare: { left: { step: prepare, path: $.stdout.text | fromjson | .mode }, op: eq, right: collect }
 *         - compare: { left: { step: prepare, path: $.stdout.text | fromjson | .needs_listing }, op: eq, right: true }
 *   save_pending:
 *     type: process.exec
 *     depends_on: [list_messages, prepare]
 *     argv:
 *     - deno
 *     - run
 *     - --allow-env
 ...
 *     depends_on: [save_pending]
 *     for_each: $.stdout.text | fromjson | .message_ids
 *     max_concurrency: 8
 *     params: { userId: me, id: $item, format: full }
 *   details_pending:
 *     endpoint: adapter/gmail
 *     method: gmail.users.messages.get
 *     depends_on: [prepare, auth_gmail]
 ...
 *         all:
 *         - compare: { left: { step: prepare, path: $.stdout.text | fromjson | .mode }, op: eq, right: collect }
 *         - compare: { left: { step: prepare, path: $.stdout.text | fromjson | .needs_listing }, op: eq, right: false }
 *   calendar_snapshot:
 *     endpoint: adapter/calendar
 *     method: calendar.events.list
 *     depends_on: [prepare, auth_calendar]
 *     params:
 ...
 *     type: process.exec
 *     depends_on: [prepare]
 *     argv: [deno, run, --allow-env, --allow-read, "@resource{validate-decisions.ts}", $decisions_json, $calendar_id]
 *     execution:
 *       mode: deferred
 *       condition:
 *         compare: { left: { step: prepare, path: $.stdout.text | fromjson | .mode }, op: eq, right: apply }
 *   lookup_owned:
 *     endpoint: adapter/calendar
 *     method: calendar.events.list
 *     depends_on: [validate_decisions, auth_calendar]
 ...
 *   plan_operations:
 *     type: process.exec
 *     depends_on: [validate_decisions, lookup_owned, lookup_nearby]
 *     argv:
 *     - deno
 *     - run
 *     - --allow-env
 ...
 * write_permissions:
 * - { tool: calendar.events.insert, adapter: calendar, mode: audit }
 * - { tool: calendar.events.patch, adapter: calendar, mode: audit }
 * writes:
...
if (mode === "collect") {
  ...
  const details = Array.isArray(detailBody) ? detailBody : [];
  ...
  const status = saved?.status === "idle"
    ? "idle"
    : pending
